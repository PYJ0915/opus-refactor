...
const ConfirmContent = ({ closeToast, title, message, onConfirm, confirmText }) => {
  ...
  return (
    <div style={{ padding: "4px" }}>
      <p style={{
        fontWeight: "500", fontSize: "16px", marginBottom: "16px",
      ...
      {message && (
        <p style={{
          fontSize: "14px", color: "#666", marginBottom: "16px",
          whiteSpace: "pre-wrap"
        }}>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <button
          ...
          style={{
            flex: 1, padding: "10px", cursor: "pointer", background: "#000",
...
export const showConfirm = (title, message, onConfirm, confirmText = "확인") => {
  // 기존에 떠있는 일반 토스트(autoClose가 있는 것들) 즉시 닫기
  toast.dismiss();

  // confirm 토스트는 별도 ID로 관리
  toast(
    ({ closeToast }) => (
      <ConfirmContent
        title={title}
